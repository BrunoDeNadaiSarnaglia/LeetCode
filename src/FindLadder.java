...
//        System.out.println(commomWords);
//        System.out.println(halfPaths(listBegin, commomWords));
//        System.out.println(halfPaths(listEnd, commomWords));
        HashMap<String, List<List<String>>> beginHalf = halfPaths(listBegin, commomWords);
        HashMap<String, List<List<String>>> endHalf = halfPaths(listEnd, commomWords);
        List<List<String>> answer = new ArrayList<List<String>>();
        for (String str : beginHalf.keySet()){
            if(!endHalf.containsKey(str)) continue;
            for (List<String> secondHalf : endHalf.get(str)){
                for (List<String> firstHalf : beginHalf.get(str)){
                    List<String> path = new ArrayList<String>();
                    path.addAll(firstHalf);
                    Collections.reverse(secondHalf);
                    path.addAll(secondHalf);
                    Collections.reverse(secondHalf);
                    answer.add(path);
                }
            }
    ...
    private HashMap<String, List<List<String>>> halfPaths(List<HashSet<String>> listSet, HashSet<String> origin){
        List<List<String>> lists = new ArrayList<List<String>>();
        for (String str1 : origin) {
            List<String> path = new ArrayList<String>();
            path.add(str1);
            lists.add(path);
        }
        for (int i = listSet.size() - 1; i >= 0; i--) {
            List<List<String>> newLists = new ArrayList<List<String>>();
            HashSet<String> set = listSet.get(i);
            for (List<String> path : lists){
                for (String str2 : this.graph.get(path.get(0))){
                    if(set.contains(str2)){
                        List<String> newPath = new ArrayList<String>();
                        newPath.add(str2);
                        newPath.addAll(path);
                        newLists.add(newPath);
                    }
                }
            }
            lists = newLists;
        ...
        for (List<String> path : lists){
            String lastOne = path.get(path.size() - 1);
            map.put(lastOne, map.getOrDefault(lastOne, new ArrayList<List<String>>()));
            map.get(lastOne).add(path);
        }
        return map;
    }

    HashSet<String> commomWords = new HashSet<String>();
    HashMap<String, List<String>> graph;

    private HashSet<String> iterate(HashSet<String> set, HashSet<String> bigSet, HashSet<String> otherBigSet){
        HashSet<String> nextSet = new HashSet<String>();
        for (String str1 : set){
            if(!graph.containsKey(str1)) continue;
            for (String str2 : graph.get(str1)){
                if(bigSet.contains(str2))  continue;
                if(otherBigSet.contains(str2))  commomWords.add(str2);
                bigSet.add(str2);
                nextSet.add(str2);
            }
        }
    ...
    private HashMap<String, List<String>> getGraph(Set<String> wordList){
        HashMap<String, List<String>> graph = new HashMap<String, List<String>>();
        for (String str1 : wordList){
            for (String str2 : wordList){
                if(almostEqual(str1, str2)){
                    graph.put(str1, graph.getOrDefault(str1, new ArrayList<String>()));
    ...
    private boolean almostEqual(String str1, String str2){
        int count = 0;
        for (int i = 0; i < str2.length(); i++) {
            if(str2.charAt(i) != str1.charAt(i) && ++count > 1) return false;
        }
        return count == 1;
    }
}
